...
export default defineComponent({
    ...
    methods: {
        getStatus(): void {
            axios.get('/api/v1/return-status/' + this.userId).then((response) => {
        ...
        create(actions): void {
            this.isLoading = true;
            axios.post('/api/v1/return-status', this.form).then(async (response) => {
        ...
        update(actions): void {
            this.isLoading = true;
            axios.put('/api/v1/return-status', this.form).then(async (response) => {
        ...
        responseHandle(response, actions, successStatus): void {
            ...
            if (response.status === successStatus) {
                this.$router.push({name: 'return-status'});
            } else {
